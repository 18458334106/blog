## 微信小程序分享朋友圈单页模式适配[微信小程序]
由于微信小程序单页模式禁用了    登录    能力
使得分享出去的页面访问接口时渲染页面拿不到数据
分享一下方案：
    1.在接口请求处获取一下当前场景值，使用  wx.getLaunchOptionsSync()  返回结果中的scene即为场景值，处理场景值为1154（即单页模式时）使用一个通用的token来访问后端接口
